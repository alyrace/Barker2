...
export default function NewCardButton({ projectId }) {
  ...
  function addNewCardHandler() {
    ...
    if (
      projectsState.projects.filter((project) => project._id === projectId)[0]?.cards.length >
      0
    ) {
      nextPosition =
        Math.max(
    ...
    projectsActions.addCardToProject({
      title: newCardName,
      projectId,
